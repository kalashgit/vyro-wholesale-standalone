...
import { ArrowRight, Check, ChevronDown, Clock, Globe2, Menu, MessageCircle, Package, ShieldCheck, Sparkles, X } from 'lucide-react'
...
const products = [
  { key: 'pro', name: 'PS5 Pro', greek: 'PS5 Pro', price: '€750.00', note: 'Performance flagship', greekNote: 'Ναυαρχίδα επιδόσεων', link: stripeLinks.pro, tone: 'pro', image: '/images/ps5-pro.png' },
  { key: 'digital', name: 'PS5 Digital Edition', greek: 'PS5 Digital Edition', price: '€489.99', note: 'All-digital next gen', greekNote: 'Ψηφιακή εμπειρία νέας γενιάς', link: stripeLinks.digital, tone: 'digital', image: '/images/ps5-digital.png' },
  { key: 'slimDisc', name: 'PS5 Slim Disc Edition', greek: 'PS5 Slim Disc Edition', price: '€549.99', note: 'Slimline with disc drive', greekNote: 'Λεπτή έκδοση με drive', link: stripeLinks.slimDisc, tone: 'disc', image: '/images/ps5-slim-disc.png' },
  { key: 'slimDigital', name: 'PS5 Slim Digital', greek: 'PS5 Slim Digital', price: '€490.00', note: 'Slimline digital', greekNote: 'Λεπτή ψηφιακή έκδοση', link: stripeLinks.slimDigital, tone: 'slim', image: '/images/ps5-slim-digital.png' },
]
...
export default function Page() {
  ...
  const [offerEnd] = useState(() => Date.now() + 2 * 24 * 60 * 60 * 1000)
  const [timeLeft, setTimeLeft] = useState(() => Math.max(offerEnd - Date.now(), 0))

  useEffect(() => {
    const interval = setInterval(() => setTimeLeft(Math.max(offerEnd - Date.now(), 0)), 1000)
  ...
  const countdown = useMemo(() => {
    ...
    return { days, hours, minutes, seconds, label: `${days}d ${pad(hours)}h ${pad(minutes)}m ${pad(seconds)}s` }
  }, [timeLeft])
  ...
  return <main className="site-shell">
    ...
    <section className="section catalog-section" id="catalog"><div className="section-heading"><div><div className="eyebrow"><span className="eyebrow-line" />{copy.catalogEyebrow}</div><h2>{copy.catalogTitle}</h2></div><p>{copy.catalogText}</p></div><div className="offer-banner"><div className="offer-badge"><Clock size={13} />{copy.offerBadge}</div><div className="offer-copy"><strong>{copy.offerTitle}</strong><span>{copy.offerText}</span></div><div className="offer-countdown"><div className="offer-unit"><strong>{String(countdown.days).padStart(2, '0')}</strong><small>{copy.offerDays}</small></div><div className="offer-unit"><strong>{String(countdown.hours).padStart(2, '0')}</strong><small>{copy.offerHours}</small></div><div className="offer-unit"><strong>{String(countdown.minutes).padStart(2, '0')}</strong><small>{copy.offerMinutes}</small></div><div className="offer-unit"><strong>{String(countdown.seconds).padStart(2, '0')}</strong><small>{copy.offerSeconds}</small></div></div></div><div className="product-grid">{products.map((product, index) => <article className={`product-card product-${product.tone}`} key={product.key}><div className="product-visual"><img className="product-image" src={product.image} alt={`${product.name} console and controller`} /><span className="product-index">0{index + 1}</span></div><div className="product-info"><div><h3>{isGreek ? product.greek : product.name}</h3><p>{isGreek ? product.greekNote : product.note}</p></div><div className="price"><small>{copy.exVat}</small><strong>{product.price}</strong></div></div><a className="product-link" href={product.link} target="_blank" rel="noreferrer">{copy.buy}<ArrowRight size={15} /></a></article>)}</div></section>
    <section className="section wholesale-section" id="wholesale"><div className="section-heading"><div><div className="eyebrow"><span className="eyebrow-line" />{copy.wholesaleEyebrow}</div><h2>{copy.wholesaleTitle}</h2></div><p>{copy.wholesaleText}</p></div><div className="offer-banner"><div className="offer-badge"><Clock size={13} />{copy.offerBadge}</div><div className="offer-copy"><strong>{copy.offerTitle}</strong><span>{copy.offerText}</span></div><div className="offer-countdown"><div className="offer-unit"><strong>{String(countdown.days).padStart(2, '0')}</strong><small>{copy.offerDays}</small></div><div className="offer-unit"><strong>{String(countdown.hours).padStart(2, '0')}</strong><small>{copy.offerHours}</small></div><div className="offer-unit"><strong>{String(countdown.minutes).padStart(2, '0')}</strong><small>{copy.offerMinutes}</small></div><div className="offer-unit"><strong>{String(countdown.seconds).padStart(2, '0')}</strong><small>{copy.offerSeconds}</small></div></div></div><div className="tier-grid">{tiers.map((tier) => <button key={tier.name} className={`tier-card ${selectedTier === tier.name ? 'is-selected' : ''}`} onClick={() => setSelectedTier(tier.name)}><span className="tier-check">{selectedTier === tier.name ? <Check size={15} /> : tier.name.slice(0, 1)}</span><span className="tier-name">{tier.name}</span><strong>{tier.range}</strong><span className="tier-description">{isGreek ? tier.greek : tier.desc}</span>{selectedTier === tier.name && <span className="selected-label">{copy.selected}</span>}</button>)}</div><button className="button button-primary tier-cta" onClick={() => scrollTo('inquiry')}>{copy.ask}<ArrowRight size={17} /></button></section>
